...
func TestKeeperCatalogPublisherIsIdempotentAndConflictSafe(t *testing.T) {
	...
	if first.Idempotent || first.DefinitionCount != 10 || first.MappingCount != 10 || first.ComponentCount != 48 {
	...
	if !second.Idempotent || second.DefinitionCount != 10 || second.MappingCount != 10 || second.ComponentCount != 48 {
	...
	var definitions, mappings, components int
	...
	`).Scan(&definitions, &mappings, &components); err != nil {
		t.Fatalf("count persisted catalog rows: %v", err)
	}
	if definitions != 10 || mappings != 10 || components != 48 {
		t.Fatalf("persisted counts = definitions:%d mappings:%d components:%d", definitions, mappings, components)
	}
	...
	if v2.Idempotent || v2.DefinitionCount != 13 || v2.MappingCount != 13 || v2.ComponentCount != 59 {
